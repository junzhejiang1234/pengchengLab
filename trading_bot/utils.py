...
def get_stock_data(stock_file):
    ...
    ## zy change
    # return df.loc[:, "Open":].values

    # standardize first
    result = df.loc[:, "Open":].values
    # result[:, :3] = (result[:, :3] - np.mean(result[:, :3], axis = 0)) / np.std(result[:, :3], axis = 0)
    # result[:, 4:] = (result[:, 4:] - np.mean(result[:, 4:], axis = 0)) / np.std(result[:, 4:], axis = 0)
    return result
